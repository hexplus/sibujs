 ...
 * that single read for later state probes.
 *
 * The promise follows the same rules as `adoptThenable()` (one `then` read,
 * invoked in a later microtask, settles at most once, a throwing `then` rejects).
 * This is the heavier variant, for callers that must also decide whether the
 * thenable had already failed — `globalStore` middleware continuations. It
 * lives in its own module so the lean `adoptThenable()`, which core's
 * `transition()` ships in the default bundle, does not carry it.
 *
...
export function adopt(value: unknown): Adoption | null {
  ...
  const follow = (result: unknown): void => {
    if (result === promise) {
      fail(new TypeError("adopt: a thenable resolved to its own adoption"));
      return;
    }
